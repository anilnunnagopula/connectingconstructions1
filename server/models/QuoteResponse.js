...
const quoteResponseItemSchema = new mongoose.Schema({
  name: {
    type: String,
    required: true,
  },
...
// ===== INDEXES =====
quoteResponseSchema.index({ quoteRequest: 1, supplier: 1 }, { unique: true });
quoteResponseSchema.index({ supplier: 1, createdAt: -1 });
quoteResponseSchema.index({ status: 1 });
quoteResponseSchema.index({ validUntil: 1 });
...
quoteResponseSchema.pre("save", async function (next) {
  if (this.isNew) {
    const QuoteRequest = mongoose.model("QuoteRequest");
    const request = await QuoteRequest.findById(this.quoteRequest);

    if (request && this.items.length !== request.items.length) {
      return next(new Error("Response items must match request items count"));
    }
  }
  next();
});
