...
quoteRequestSchema.index({ targetSuppliers: 1 });
quoteRequestSchema.index({ expiresAt: 1 });
quoteRequestSchema.index({ broadcastToAll: 1, status: 1 });

// ===== VIRTUALS =====
quoteRequestSchema.virtual("quoteNumber").get(function () {
  return `QT-${this._id.toString().slice(-8).toUpperCase()}`;
});
...
quoteRequestSchema.virtual("daysUntilRequired").get(function () {
  const diff = this.requiredBy - new Date();
