...
orderSchema.virtual("canBeCancelled").get(function () {
  return !this.isDeleted && ["Pending", "Processing"].includes(this.orderStatus) && this.paymentStatus !== "paid";
}); // Check if order is active

orderSchema.virtual("isActive").get(function () {
  return !this.isDeleted && !["Cancelled", "Delivered", "Refunded"].includes(this.orderStatus);
}); // ===== METHODS =====
// Soft delete method
...
orderSchema.methods.getSuppliers = function () {
  return _toConsumableArray(new Set(this.products.map(function (p) {
    return p.supplier.toString();
  })));
...
orderSchema.methods.getProductsBySupplier = function (supplierId) {
  return this.products.filter(function (p) {
    return p.supplier.toString() === supplierId.toString();
  });
...
orderSchema.pre("save", function (next) {
  this.updatedAt = Date.now(); // Add initial status to history if this is a new order
...
orderSchema.query.active = function () {
  return this.where({
    isDeleted: false,
    orderStatus: {
      $nin: ["Cancelled", "Delivered", "Refunded"]
    }
  });
};
